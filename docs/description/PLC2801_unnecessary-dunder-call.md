...
Types may implement only a subset of related dunder methods. Calling a
...
Instance-assigned dunder methods are ignored by operators and builtins.
class C: pass
c = C()
c.__bool__ = lambda: False
c.__bool__() # before fix: False
bool(c)      # after fix: True
Even with built-in types, behavior can differ.
